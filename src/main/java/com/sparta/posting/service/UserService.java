...
public class UserService {
    ...
    @Transactional
    public HttpResponseDto signup(SignupRequestDto signupRequestDto) throws HttpResponseDto {
    ...
    public HttpResponseDto login(LoginRequestDto loginRequestDto, HttpServletResponse response) throws HttpResponseDto {
        String username = loginRequestDto.getUsername();
        String password = loginRequestDto.getPassword();

        User user = userRepository.findByUsername(username).orElseThrow(
                () -> new HttpResponseDto("회원을 찾을 수 없습니다.", 400)
        );
